...
def get_spi_tickers_from_web() -> list[str]:
    ...
    """
    try:
        tables = pd.read_html(SPI_WIKI_URL)
        for tbl in tables:
            if "Symbol" in tbl.columns:
                ...
                # enlever doublons / vides
                tickers = sorted({t for t in tickers if t})
                return tickers
    except Exception as e:
        st.warning(f"Impossible de charger la liste SPI en ligne : {e}")

    # Fallback
    st.info("Utilisation de la liste SMI par défaut.")
